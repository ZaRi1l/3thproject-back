...
|김동현|프론트엔드: REST/GraphQL API를 연동하여 사용자 인증 흐름(로그인, 회원 생성) 및 환자 검색 기능을 개발하고, React Context API를 활용한 전역 로그인 상태 관리 구현 <br>백엔드: 애플리케이션 DB 설계(PostgreSQL), JWT와 Spring Security를 활용한 회원 인증 REST API 구축, GraphQL Resolver를 통해 이종 데이터베이스(Oracle, PostgreSQL)의 데이터를 조합하는 소견서 작성 API 개발 <br>기타: 발표 자료 제작 |
